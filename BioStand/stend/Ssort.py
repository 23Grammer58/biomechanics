...
def calibration_params(name_protocol):
    ...
    images = glob.glob(r'C:\Users\Lenovo\PycharmProjects\MainBioStand\BioStand\files\Calibration4\*.jpg')
    # print(images)
...
class Sort:
            ...
            if "exp_params" not in path_element_Inication:
                with open(path_element_Inication, 'r') as f:
                    try:
                        ...
                        logging.error(f"Error  write frame {id}")
                        id += 1
                        continue
                    try:
                        value_key = dictionary_string[f'{id}']
                    except KeyError as err:
                        logging.error(f"Skip frame {id}")
                        id += 1
                        continue
    ...
    def look(self,frame, index:int):
        npy = np.load(frame).astype(np.uint8)  # read npy
    ...
    def conversion_to_grayscale(self,frame, index:int, look_image = False):
        ...
        np.save(fr'{self.paths[3]}/{index}', frame)

        if look_image:
            frame = frame.astype(np.uint8)
            frame = Image.fromarray(frame)
            frame.save(fr'{folder_location_image}/{index}.tif')
    ...
    def draw_plot(self,):
        ...
        y = self.data.get('mean_tenzo_1_3')
...
if __name__ == "__main__":
    ...
    path = r'C:\Users\Lenovo\PycharmProjects\MainBioStand\BioStand\files\VHB49102022mm'
    name_protocol = 'VHB49102022mm'
    look_images = list(range(1,400))
