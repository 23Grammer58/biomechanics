...
class Sort:
    def __init__(self, path: str, name_protocol: str, calib_params: dict or None, look_image_list: List = []):
        self.path = path
    ...
    def draw_plot(self):
        ...
        '''
        x = self.data.get('axis_1_3')
        y = self.data.get('correlation_force_1_3')
        # print('x',len(x))
        # print('y',y)
...
class Calib:
    ...
    @staticmethod
    def read_json(path:Path or str):
    ...
    def calibration_params(name_protocol: str, path: Path, chessboardSize: tuple):
        ...
        size_of_chessboard_squares_mm = 1.5 #выбрать размер доски
        ...
        images = glob.glob('C:/Users/Lenovo/PycharmProjects/MainBioStand/BioStand/files/Calibration4/*.jpg')
        # print('ss',images)
...
if __name__ == "__main__":
    ...
    path = 'C:/Users/Lenovo/PycharmProjects/MainBioStand/BioStand/files/eco_01Hor2'  # задаем путь в нашу папочку с данными
    name_protocol = 'eco_01Hor2'  # задаем имя нашего протокола
    look_images = list(range(1, 80))  # можем записать номера изображений которые хотим глянуть
